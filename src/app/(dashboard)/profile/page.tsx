...
export default function ProfilePage() {
  ...
  const STATS = [
    ...
    { label: "Day Streak", value: `${(user as any)?.streak ?? 0}`, icon: Flame, color: "text-orange-400" },
  ];
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0 max-w-2xl">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
        ...
        {error && <div className="mb-4 px-4 py-3 rounded-xl bg-red-900/30 border border-red-500/30 text-red-400 text-sm">{error}</div>}
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <input value={form.jobTitle} onChange={e => setForm(f => ({...f, jobTitle: e.target.value}))} placeholder="e.g. Senior Policy Analyst"
          ...
          <div>
            ...
            <textarea value={form.bio} onChange={e => setForm(f => ({...f, bio: e.target.value}))} rows={3}
              placeholder="Tell others about yourself..." className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none"/>
          </div>
          <div>
            <label className="text-slate-400 text-xs mb-1.5 block">Email (read-only)</label>
            <input value={user?.email ?? ""} readOnly className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-slate-700/50 text-slate-500 text-sm cursor-not-allowed"/>
          </div>
